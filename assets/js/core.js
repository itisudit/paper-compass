...
function ensureAnnotationModal(){
  ...
  modal.hidden=true;
  modal.innerHTML=`
    <div class="annotation-modal-backdrop" data-ann-close></div>
    <section class="annotation-modal-card" role="dialog" aria-modal="true" aria-labelledby="annotationModalTitle">
      <div class="annotation-modal-head">
        <div><p class="annotation-modal-kicker">Linked annotation</p><h3 id="annotationModalTitle">Edit annotation</h3></div>
        <button type="button" class="annotation-close" data-ann-close aria-label="Close annotation">×</button>
      </div>
      <label>Selected excerpt<textarea id="annotationEditText" rows="8" placeholder="Edit the saved excerpt if the PDF selection missed part of the highlighted text."></textarea></label>
      <label>Your comment<textarea id="annotationEditNote" rows="4" placeholder="Optional comment on this excerpt…"></textarea></label>
      <label>Link to<select id="annotationEditLink">
        <option value="">Unlinked</option>
        <option value="note-s1">Step 1 · Overview</option><option value="note-s2">Step 2 · Core question</option>
        <option value="note-s3">Step 3 · Knowledge gap</option><option value="note-s4">Step 4 · Methods & power</option>
        <option value="note-s5">Step 5 · My conclusion</option><option value="note-s6">Step 6 · Reconciliation</option>
        <option value="note-s7">Step 7 · Confounders & limits</option><option value="evidence">Evidence table</option><option value="claim">Claim matrix</option>
      </select></label>
      <div class="annotation-modal-actions">
        <button type="button" class="delete" id="annotationModalDelete">Delete</button>
        <span></span>
        <button type="button" class="secondary" data-ann-close>Cancel</button>
        <button type="button" id="annotationModalSave">Save changes</button>
      </div>
    </section>`;
  document.body.appendChild(modal);
  modal.querySelectorAll('[data-ann-close]').forEach(b=>b.onclick=()=>{ modal.hidden=true; modal.dataset.annotationId=''; });
  return modal;
}
function openAnnotationEditor(id){
  const item=(data.annotations||[]).find(a=>a.id===id); if(!item) return;
  const modal=ensureAnnotationModal();
  modal.dataset.annotationId=id;
  $('#annotationEditText').value=item.text||'';
  $('#annotationEditNote').value=item.note||'';
  $('#annotationEditLink').value=item.linkTo||'';
  $('#annotationModalSave').onclick=()=>{
    const target=(data.annotations||[]).find(a=>a.id===id); if(!target) return;
    target.text=$('#annotationEditText').value.trim();
    target.note=$('#annotationEditNote').value;
    target.linkTo=$('#annotationEditLink').value;
    save('Annotation updated · syncing…');
    modal.hidden=true;
  };
  $('#annotationModalDelete').onclick=()=>{
    data.annotations=(data.annotations||[]).filter(a=>a.id!==id);
    save('Annotation deleted · syncing…');
    modal.hidden=true;
  };
  modal.hidden=false;
  $('#annotationEditText').focus();
}
function renderAnnotations(){
  const list=$('#annotationList'); if(!list) return;
  const anns=data.annotations||[];
  const count=$('#annotationCount'); if(count) count.textContent=String(anns.length);
  if(!anns.length){ list.innerHTML='<p class="annotation-empty">Saved excerpts will appear here. Click one to open, edit, or relink it.</p>'; return; }
  list.innerHTML=anns.map(a=>{
    const preview=(a.text||'').replace(/\s+/g,' ').trim();
    const short=preview.length>150?preview.slice(0,150)+'…':preview;
    return `<button type="button" class="annotation-chip" data-ann-open="${a.id}" title="Open annotation"><span class="tag">${esc(annotationTargetLabel(a.linkTo))}</span><span class="annotation-chip-text">${esc(short)}</span><span class="annotation-chip-meta">p.${a.page||'?'} · Open/edit</span></button>`;
  }).join('');
  list.querySelectorAll('[data-ann-open]').forEach(b=>b.onclick=()=>openAnnotationEditor(b.dataset.annOpen));
}
...
// ---- Deep Dive layout: read and write side-by-side on larger screens ----
(function arrangeDeepDiveWorkspace(){
  const pdfWorkspace=document.querySelector('.pdf-workspace');
  const firstPhase=document.querySelector('.phase');
  const nav=document.querySelector('.step-navigator');
  if(!pdfWorkspace||!firstPhase||!nav) return;
  const parent=pdfWorkspace.parentNode;
  if(document.querySelector('.deep-dive-grid')) return;
  const grid=document.createElement('div');
  grid.className='deep-dive-grid';
  const reading=document.createElement('div'); reading.className='deep-dive-reading';
  const thinking=document.createElement('div'); thinking.className='deep-dive-thinking';
  parent.insertBefore(grid,pdfWorkspace);
  grid.appendChild(reading); grid.appendChild(thinking);
  reading.appendChild(pdfWorkspace);
  thinking.appendChild(nav);
  const phases=[...document.querySelectorAll('.phase')];
  phases.forEach(phase=>thinking.appendChild(phase));
})();
